...
{
    auto xLeft = begin.x();
    auto ybottom = begin.y();
    auto xRight = end.x();
    auto yTop = end.y();

    Eigen::Vector3f line_color = {255, 255, 255};

    int x,y,dx,dy,dxLeft,dybottom,px,py,xe,ye,i;

    dx=xRight-xLeft;
    dy=yTop-ybottom;
    dxLeft=fabs(dx);
    dybottom=fabs(dy);
    px=2*dybottom-dxLeft;
    py=2*dxLeft-dybottom;
    ...
    if (SSAA) {
        ...
        for (int y = (int)(floor(yBottom)); y < (int)(ceil(yTop)); ++y) {
            for (int x = (int)(floor(xLeft)); x < (int)(ceil(xRight)); ++x) {
                ...
                if (ras == 4) {
                    set_pixel(Vector2i(x, y), color / 4.0 /* + frame_buf[get_index(x, y)]*/);  // 综合多轮渲染的结果
                } else if (ras >= 1) {
                    set_pixel(Vector2i(x, y), color / 4.0 + frame_buf[get_index(x, y)] * (4-ras/4));  // 综合多轮渲染的结果
                }
            }
        }
    } else {
...
{
    // 当buff最低位为1时，清空帧缓存，全部设置为黑色
    if ((buff & rst::Buffers::Color) == rst::Buffers::Color)
    {
        std::fill(frame_buf.begin(), frame_buf.end(), Eigen::Vector3f{0, 0, 0});
    }
    // 当buff次低位为1时，清空深度缓存，深度值设置为无穷远
    // 存在错误
