                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    Vector3f pvec_s1 = crossProduct(dir, edge2);
    float det = dotProduct(edge1, pvec_s1);
    if (det == 0 || det < 0)
        return false;

    Vector3f tvec_s = orig - v0;
    u = dotProduct(tvec_s, pvec_s1);
    if (u < 0 || u > det)
        return false;

    Vector3f qvec_s2 = crossProduct(tvec_s, edge1);
    v = dotProduct(dir, qvec_s2);
    if (v < 0 || u + v > det)
        return false;

    float invDet = 1 / det;

    tnear = dotProduct(edge2, qvec_s2) * invDet;
    u *= invDet;
    v *= invDet;
